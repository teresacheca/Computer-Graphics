...
public:
    ...
    Hit intersect(Ray ray){
		...
		*/
		...
		i0 = (dot_product_d_c + sqrt(pow(radius,2) - pow(normal_c,2) + pow(dot_product_d_c, 2))) *ray.direction[0] + ray.origin[0];
		i1 = (dot_product_d_c + sqrt(pow(radius,2) - pow(normal_c,2) + pow(dot_product_d_c, 2))) *ray.direction[1] + ray.origin[1];
		i2 = (dot_product_d_c + sqrt(pow(radius,2) - pow(normal_c,2) + pow(dot_product_d_c, 2))) *ray.direction[2] + ray.origin[2];
		glm::vec3 gamma_t1 = {i0,i1,i2};

		p0 = (dot_product_d_c - sqrt(pow(radius,2) - pow(normal_c,2) + pow(dot_product_d_c, 2))) *ray.direction[0] + ray.origin[0];
		p1 = (dot_product_d_c - sqrt(pow(radius,2) - pow(normal_c,2) + pow(dot_product_d_c, 2))) *ray.direction[1] + ray.origin[1];
		p2 = (dot_product_d_c - sqrt(pow(radius,2) - pow(normal_c,2) + pow(dot_product_d_c, 2))) *ray.direction[2] + ray.origin[2];
		glm::vec3 gamma_t2 = {p0,p1,p2};	
		...
		//For gamma_t1 and gamma_t2:
		glm::vec3 normal1, normal2;

		normal1 = glm::normalize(gamma_t1),
		normal2 = glm::normalize(gamma_t2);
		...
		distance1 = sqrt( pow(gamma_t1[0] - ray.origin[0], 2) + pow(gamma_t1[1] - ray.origin[1], 2) + pow(gamma_t1[2] - ray.origin[2], 2));
		distance2 = sqrt( pow(gamma_t2[0] - ray.origin[0], 2) + pow(gamma_t2[1] - ray.origin[1], 2) + pow(gamma_t2[2] - ray.origin[2], 2));

		if(distance1 <= distance2){
			hit.intersection = gamma_t1;
			hit.normal= normal1;
			hit.distance = distance1;

		}else{
			hit.intersection = gamma_t2;
			hit.normal = normal2;
