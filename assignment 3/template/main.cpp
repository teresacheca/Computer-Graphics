...
public:
    ...
    Hit intersect(Ray ray){
        ...
        if(D<=radius){
			...
			float theta, gamma;
			float pi = (float) M_PI;
			theta = asin((hit.normal[1]/radius)*2*pi);			//multiplied by 2 to get more curvature
			theta = glm::clamp(theta, -pi/2.0f, pi/2.0f);

			gamma = atan2(hit.normal[2]*2*pi, hit.normal[0]*2*pi);
			gamma = glm::clamp(gamma, -pi, pi);

			hit.uv.s = (gamma + pi) / (2.0f*pi);
			hit.uv.t = (theta + (pi/2.0f))/pi;
		
			
			hit.uv.s = glm::clamp(hit.uv.s, 0.0f, 1.0f);
			hit.uv.t = glm::clamp(hit.uv.t, 0.0f, 1.0f);
...
public:
	...
	Hit intersect(Ray ray){
		 ...
		 */
		float t;
		float aux1 = glm::dot(point - ray.origin, normal);
		
		float aux2= glm::dot(ray.direction, normal);
...
glm::vec3 PhongModel(glm::vec3 point, glm::vec3 normal, glm::vec2 uv, glm::vec3 view_direction, Material material){
	...
	for(int light_num = 0; light_num < lights.size(); light_num++){
		...
		distance= glm::clamp(distance, 0.0f,1.5f);
		
		float distance2=pow(distance,2);
		float a1=0.1;
		float a2=0.2;
		float a3=0.3;
		float att=1/(a1+a2*distance+a3*distance2);
...
void sceneDefinition (){
	...
	Material textured;				
	//textured.texture = &checkerboardTexture;				//We solved both textures
	...
	Material auxiliar;
	auxiliar.ambient = glm::vec3(0.8f, 0.8f, 0.8f);

	Material auxiliar2;
	auxiliar2.ambient = glm::vec3(0.7f, 0.5f, 0.5f);

	Material auxiliar3;
	auxiliar3.ambient = glm::vec3(0.7f, 0.8f, 0.1f);

	Material auxiliar4;
	auxiliar4.ambient = glm::vec3(0.7f, 0.7f, 1.0f);

	

	objects.push_back(new Plane(glm::vec3 (0,-3,0), glm::vec3 (0,1,0), auxiliar));		//plano de abajo --> abajo
	objects.push_back(new Plane(glm::vec3 (15,0,0), glm::vec3 (1,0,0), auxiliar3));		// plano de la derecha
	objects.push_back(new Plane(glm::vec3 (0,0,-0.01), glm::vec3 (0,0,1), auxiliar2));	//plano de frente
	objects.push_back(new Plane(glm::vec3 (0,27,0), glm::vec3 (0,1,0), auxiliar2));			//plano de arriba
	objects.push_back(new Plane(glm::vec3 (-15,0,0), glm::vec3 (1,0,0), auxiliar4));		// plano de la izquierda
	objects.push_back(new Plane(glm::vec3 (0,0,30), glm::vec3 (0,0,1), auxiliar2));			// plano de fondo
...
glm::vec3 toneMapping(glm::vec3 intensity){
	...
	float aux0=pow(intensity[0],beta);
	aux0=a*aux0;
	aux0=pow(aux0,(1/gamma));
	tonemapped[0]=min(aux0,1.0f);

	float aux1=pow(intensity[1],beta);
	aux1=a*aux1;
	aux1=pow(aux1,(1/gamma));
	tonemapped[1]=min(aux1,1.0f);

	float aux2=pow(intensity[2],beta);
	aux2=a*aux2;
	aux2=pow(aux2,(1/gamma));
	tonemapped[2]=min(aux2,1.0f);
