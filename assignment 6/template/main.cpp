...
glm::vec3 PhongModel(glm::vec3 point, glm::vec3 normal, glm::vec2 uv, glm::vec3 view_direction, Material material){
	...
	glm::vec3 color(0.0);
		...
		//exercise 1
		Ray shadow(point, light_direction);
		int shadowterm=1;
		for(int k = 0; k<objects.size(); k++){
		
		Hit hit = objects[k]->intersect(shadow);
		if(hit.hit == true  && hit.distance<glm::distance(lights[light_num]->position, point) && hit.distance>0.001 ) {
				shadowterm=0;
		}
 ...
 */
glm::vec3 trace_ray(Ray ray, int a){
	...
	//exercise 2
	if(closest_hit.object->getMaterial().refleja && a<15 ){
		glm::vec3 reflectiondir=glm::reflect(closest_hit.intersection,closest_hit.normal);
		reflectiondir=glm::normalize(reflectiondir);
		Ray reflectionray(closest_hit.intersection, reflectiondir);
		reflectioncolor=trace_ray(reflectionray,a+1);

		color=color+reflectioncolor*closest_hit.object->getMaterial().alp;
	
	}

	//exercise 3
	if(closest_hit.object->getMaterial().refrac && a<15 ){
		...
		refractiondir=glm::refract(ray.direction,normal,sigma);
		refractiondir=glm::normalize(refractiondir);
		
		flag=!flag;	
		Ray refractionray(closest_hit.intersection, refractiondir);
		refractioncolor=trace_ray(refractionray,a+1);

		color=color+refractioncolor;
...
void sceneDefinition (){
	...
	blue_specular.diffuse = glm::vec3(0.1);
	blue_specular.alp=glm::vec3(0.5);
	blue_specular.refleja=true;

	Material refraction_effect;
	refraction_effect.refindex=2;
	refraction_effect.refrac=true;
	refraction_effect.diffuse=glm::vec3(0.1);

	objects.push_back(new Sphere(1.0, glm::vec3(1,-2,8), blue_specular));
	objects.push_back(new Sphere(0.5, glm::vec3(-1,-2.5,6), red_specular));
	objects.push_back(new Sphere(2.0, glm::vec3(-3,-1,8), refraction_effect));
